...
public class Proj1{
    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
		...
		Text word=new Text();
		String s="";
		ArrayList<Integer> tar_position=new ArrayList<Integer>();
		int counter=1;
		while (matcher.find()){
		    if (matcher.group().toLowerCase().equals(targetGram)){
			tar_position.add(counter);}
		    counter=counter+1;}
		if (tar_position.size()==0){
		    Matcher new_matcher = WORD_PATTERN.matcher(docContents.toString());
		    while (new_matcher.find()) {
			DoublePair toreturn=new DoublePair(1,0);
			word.set(new_matcher.group().toLowerCase());
			context.write(word, toreturn);
		    }
		}
		else {
		    counter=1;
		    ...
		    while (new_matcher.find()) {
			int distance=0;
			s=new_matcher.group().toLowerCase();
			if (!s.equals(targetGram)){
			    int min=Math.abs(tar_position.get(0)-counter);
			    for (int i=1;i<tar_position.size();i++){
				if (Math.abs(tar_position.get(i)-counter)<min){
				    min=Math.abs(tar_position.get(i)-counter);}
				else {
				    break;
				}
			    }
			    distance=min;
			    DoublePair toreturn=new DoublePair(1,func.f(distance));
			    word.set(s);
			    context.write(word,toreturn);
			}
			counter=counter+1;
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
		...
		// YOUR CODE HERE                                                                                                                  
            ...
            Double toreturn=0D;
	    if (sum2>0){
		toreturn=sum2 * Math.pow(Math.log(sum2),3) / sum1;}
	    context.write(new DoubleWritable((-1)*toreturn),key);
